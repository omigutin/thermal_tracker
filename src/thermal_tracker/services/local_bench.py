...
from ..connections.shared_memory import DEFAULT_FRAME_HEIGHT, DEFAULT_FRAME_WIDTH, DEFAULT_SHARED_MEMORY_PREFIX
from .shared_memory_cleanup import cleanup_shared_memory
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--config", default="configs/shared_memory_smoke.toml", help="Runtime config.")
...
def main() -> None:
    """Точка входа `python -m thermal_tracker.services.local_bench`."""
...
def _build_process_specs(args: argparse.Namespace, gateway_url: str) -> list[_ProcessSpec]:
    ...
    gateway_command = [
        sys.executable,
        "-m",
        "thermal_tracker.services.gateway_service",
        "--host",
        args.host,
        "--port",
        str(args.port),
    ...
    runtime_command = [
        sys.executable,
        "-m",
        "thermal_tracker.services.shared_memory_runtime_worker",
        "--config",
        args.config,
        "--report-every",
        "50",
    ...
    if args.video_path:
        sender_command = [
            sys.executable,
            "-m",
            "thermal_tracker.services.network_video_sender",
            args.video_path,
            "--gateway-url",
            gateway_url,
            "--fps",
        ...
        sender_command = [
            sys.executable,
            "-m",
            "thermal_tracker.services.synthetic_network_sender",
            "--gateway-url",
            gateway_url,
            "--fps",
            str(args.fps),
