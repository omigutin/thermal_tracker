...
class StageConfigParser:
    """
        Парсит одну stage-секцию пресета в объект StageConfig.

        Класс получает уже найденную секцию стадии из TOML-пресета,
        читает общий флаг enabled и список operations,
        затем для каждой операции находит нужный config-класс по полю type.

        StageConfigParser не знает бизнес-логику операций и не создаёт runtime-объекты.
        Его задача — только превратить сырые данные пресета в строгие config-объекты,
        которые дальше будут переданы менеджеру и фабрике стадии.
    """
