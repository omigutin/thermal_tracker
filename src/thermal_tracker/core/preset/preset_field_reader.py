...
class PresetFieldReader:
    """
        ...
        Класс используется на этапе сборки строгих config-объектов из TOML-данных.
        Он достаёт значения из словаря, проверяет базовые типы, переносит значения
        в kwargs и помогает обнаружить неизвестные параметры.

        PresetFieldReader не должен использоваться в runtime pipeline, managers или
        factories. После завершения парсинга код должен работать только с готовыми
        dataclass-конфигами.
    """
